...
metrics = pickle.load(open("evaluation/metrics.pkl", "rb"))

# Add metrics dashboard to sidebar
st.sidebar.title("Model Metrics")
st.sidebar.metric("Clustering Score", f"{metrics['clustering_score']:.3f}")
st.sidebar.metric("Feature Dimensions", metrics['feature_dim'])
st.sidebar.metric("Total Samples", metrics['total_samples'])

# Add evaluation plots
st.sidebar.subheader("Model Evaluation")
st.sidebar.image("evaluation/feature_distribution.png", caption="Feature Distribution")
st.sidebar.image("evaluation/similarity_heatmap.png", caption="Similarity Heatmap")

# Rest of the existing main.py code remains the same
odel = ResNet50(weights="imagenet", include_top=False, input_shape=(224, 224, 3))
